...
*Last updated: **v0.1.1 alpha tagged**. Two-tier logging (per-entry capture + end-of-day confirmation), `ACCESS.md` audit + 10 grouped a11y tracking issues, and the AGPL → GPL v3 license switch land in this release. Phases 3 – 13 complete; Phase 15 (Android shell) landed to the extent needed to ship a debug APK; Phase 16 (release engineering) delivered via the GitHub Actions release workflow. Phases 14 (polish), iOS half of Phase 15, and Phase 17 (store submission) remain and are out of scope for the alpha.*
